...
public class FileUtil {
	...
	public static String writeHtml(String html, String path, String fileName) {
		...
		String file_url = url.substring(url.lastIndexOf("/uploadfile"), url.length());
		return file_url;
	}
	...
	public static String getString(String name) {
		...
		/*
		 * File file = new File(name); Long filelength = file.length(); //获取文件长度
		 * byte[] filecontent = new byte[filelength.intValue()]; try {
		 * FileInputStream in = new FileInputStream(file); in.read(filecontent);
		 * in.close(); } catch (FileNotFoundException e) { e.printStackTrace();
		 * } catch (IOException e) { e.printStackTrace(); } return new
		 * String(filecontent);//返回文件内容,默认编码
		 */ }
}
...
var Web = {
	...
	Recource:{
		...
		serverSysURL:"http://"+serverIp+":"+serverPort+"/wfk.protocol.http/api/sys/",
		appServerURL:"http://10.36.16.61:8888/AppPlatform/page/"
	},
	Method:{
		toTxt:function (val) {
			if(val){
		...
		ajax:function(method, param) {
			...
			$.ajax({
				...
				success: function(data){
					...
					}else if(data.result ==3 && data.errcode == 399 && method.indexOf("getUserInfo") == -1){
						alert("用户为登录或失效");
...
ZeroClipboard - jQuery ZeroClipboard Plugin
-----------------------------------------------
...
(function($) {
	...
	$.fn.zclip = function(option, param){
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.zclip');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, Zclip.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.zclip', (data = new Zclip(this, options)));
...
(function ($) {
	...
	$.fn.imgpicker = function(option, param){
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.imgpicker');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, ImgPicker.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.imgpicker', (data = new ImgPicker(this, options)));
			if (typeof option == 'string') data[option](param);
		});
	};
...
(function ($) {
	...
	$.fn.strong = function(option, param){
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.strong');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, Strong.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.strong', (data = new Strong(this, options)));
			if (typeof option == 'string') data[option](param);
		});
	};
...
(function ($) {
	...
	Select.prototype.init = function() {
		if(this.options.container){
			this.select = $("<div class='dropdown dropdown-scroll'>" +
			...
			this.container.append("<a class='dropdown-toggle btn btn-default' data-toggle='dropdown'>" +
					this.options.icon+" <span>"+this.options.display+"</span> <span class='caret'></span>" +
	...
	Select.prototype.setData = function(data) {
		...
		for(var key in data){
			...
			$this.menu.append("<li><a href='javascript:;' target='"+item.value+"'>"+item.key+"</a></li>");
		}
		if(data.length > 0){
			...
			$this.menu.find("a").click($this, function(event){
				var select = event.data;
				select.menu.find("li").removeClass("active");
				$(this).parent().addClass("active");
				select.selectItem = {key:$(this).text(), value:select.items[$(this).parent().index()].value, items:select.items[$(this).parent().index()].items};
	...
	$.fn.select = function(option, param){
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.select');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, Select.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.select', (data = new Select(this, options)));
			if (typeof option == 'string') data[option](param);
		});
	};
...
(function ($) {
	...
	AreaSelect.prototype.init = function() {
		...
		$this.province.select({
			...
			click:function(ui, item){
				...
				$this.area.select("setData", citys.length>0?citys[$this.city.data("crap.select").selectItem.value].items:[]);
				$this.selectValue();
		...
		$this.city.select({
			...
			click:function(ui, item){
				$this.area.select("setData", Web.Recource.areaSelect[$this.province.data("crap.select").selectItem.value].items[item.value].items);
				$this.selectValue();
	...
	AreaSelect.prototype.setSelect = function(value) {
		...
		var citys = this.province.data("crap.select").selectItem.items;
		this.city.select("setData", citys).select("setSelect", values[1]);
		var areas = this.city.data("crap.select").selectItem.items;
	...
	AreaSelect.prototype.selectValue = function() {
		var province = this.province.data("crap.select").selectItem;
		var city = this.city.data("crap.select").selectItem;
		var area = this.area.data("crap.select").selectItem;
		this.value = {
	...
	AreaSelect.prototype.autoHide = function() {
		if(this.province.data("crap.select").items.length > 0){
			this.province.show();
		}else{
			this.province.hide();
		}
		if(this.city.data("crap.select").items.length > 0){
			this.city.show();
		}else{
			this.city.hide();
		}
		if(this.area.data("crap.select").items.length > 0){
			this.area.show();
		}else{
			this.area.hide();
		}
	...
	$.fn.areaSelect = function(option, param){
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.areaSelect');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, AreaSelect.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.areaSelect', (data = new AreaSelect(this, options)));
			if (typeof option == 'string') data[option](param);
		});
	};
...
(function ($) {
	...
	PageSet.prototype.setData = function(pageSet) {
		...
		$this.ul.append("<li class='"+(pageSet.page > 1?"enable":"disabled")+"' name='1'><a href='javascript:;'><img src='images/page_first.png'></a></a></li>");
		$this.ul.append("<li class='"+(pageSet.page > 1?"enable":"disabled")+"' name='"+(pageSet.page-1)+"'><a href='javascript:;''><img src='images/page_pre.png'></a></li>");
		...
		$this.ul.append("<li class='"+(pageSet.page < pageSet.totalpage?"enable":"disabled")+"' name='"+(pageSet.page+1)+"'><a href='javascript:;''><img src='images/page_next.png'></a></li>");
		
		$this.ul.append("<li class='"+(rightNum < pageSet.totalpage?"enable":"disabled")+"' name='"+pageSet.totalpage+"'><a href='javascript:;'><img src='images/page_last.png'></a></li>");
...
(function($) {
	...
	$.fn.treeview = function(option, param) {
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.treeview');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, TreeView.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.treeview', (data = new TreeView(this, options)));
			if (typeof option == 'string') data[option](param);
		});
	};
...
(function($) {
	...
	MsgAlert.prototype.destroy = function(){
		...
		this.alert.fadeOut(this.options.delay,function(){
			$(this).remove();
			$this.el.removeData('crap.msgAlert');
		});
		if(this.options.modal){
	...
	$.fn.msgAlert = function(option) {
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.msgAlert');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, MsgAlert.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.msgAlert', (data = new MsgAlert(this, options)));
...
(function($) {
	...
	ValidateAlert.prototype.destroy = function(){
		...
		this.alert.fadeOut(this.options.delay,function(){
			$(this).remove();
			$this.el.removeData('crap.valAlert');
		});
	};
	...
	$.fn.valAlert = function(option, param) {
		...
		return this.each(function() {
			var $this   = $(this);
			var data    = $this.data('crap.valAlert');
			var options = typeof option == 'object' && option;
			
			options = $.extend(true, {}, ValidateAlert.defaultOption, $this.data(), options);
			
			if (!data) $this.data('crap.valAlert', (data = new ValidateAlert(this, options)));
			if (typeof option == 'string') data[option](param);
		});
	};
})(jQuery);
...
(function($) {
	...
	ProgressAlert.prototype.init = function(){
		this.alert = $("<div class='alert alert-"+this.options.alert+" proAlert' style='"+this.options.style+"'>"+
				"<img src='images/preloading.gif' style='max-height: 70px'>"+
			"</div>");
		$("body").append(this.alert);

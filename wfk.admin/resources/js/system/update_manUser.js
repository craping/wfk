$(function(){
	...
	$("#userName,#email").bind("blur",function(){
		validate(this.id,0,false);
	});
	Web.Method.ajax("manUser/getUser", {
		data : {
			id : id,
		},
		success : function(data) {
			$("#login_name").val(data.info["loginName"]);
			$("#oldphone").val(data.info["phone"]);
			$(".man_details").each(function(){
	...
	$("#loginName").bind("blur",function() {
		if($("#loginName").val()==$("#login_name").val())
			return;
	...
	$(document).on("click","#update_manUser",function(){
		...
		if(!validate("userName,email",0,false)){
			return false;
		}
		var id=id;

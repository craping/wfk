$(function(){
	...
	$("#listview_returns").listview({
		...
		eachItem:function(ui, item){
			var accept_link = item.type==0?"return_retreat_accept.html?":"return_change_accept.html?";
			...
			var accept = item.status=="0"?"<li><a href='"+ accept_link + linkParams +"' target='iframe'>受理</a></li>":"";
...
function queryRuturnsList(page, listview, pageset){
	...
	Web.Method.ajax("supOrdReturn/getUserReturn",{
		data:datas,
...
function batchConfirmReturn() {
	...
	if($("input[name='returnHandler']").val() == 1){
		Web.Method.ajax("supOrdReturn/batchHandler", {
			data : {
		...
		location.href = Web.Recource.serverURL+"supOrdReturn/batchHandler?returnHandler="+$("input[name='returnHandler']").val()+"&ids="+getCheckVal();
	}
}

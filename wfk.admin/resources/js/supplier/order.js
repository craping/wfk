$(function(){
	...
	$("#listview_orders").listview({
		...
		eachItem:function(ui, item){
			...
			var edit = "<li><a href='order_edit.html?orderId="+ item.orderId +"' target='iframe'>编辑</a></li>";
			var details = "<li><a href='order_details.html?orderId="+ item.orderId +"' target='iframe'>详情</a></li>";
			var accept = item.orderStatus=="0"?"<li><a href='order_accept.html?orderId="+ item.orderId +"' target='iframe'>受理</a></li>":"";
			var delivery = item.orderStatus=="2"?"<li><a href='order_delivery.html?orderId="+ item.orderId +"' target='iframe'>发货</a></li>":"";
			...
			var tr = $("<tr>" +
						...
						"<td>"+ handle_head + edit + details + accept + delivery + handle_foot +"</td>" +
					"</tr>");
...
function queryOrdersList(page, listview, pageset){
	...
	Web.Method.ajax("supOrder/getOrderList",{
		data:datas,
...
function batchConfirmOrder() {
	...
	if($("input[name='orderHandler']").val() == 1){
		...
		location.href = Web.Recource.serverURL+"supOrder/batchHandler?orderHandler="+$("input[name='orderHandler']").val()+"&ids="+getCheckVal();
	}
}
